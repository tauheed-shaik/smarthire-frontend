...
function ManageProfile() {
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.put(
        `${import.meta.env.VITE_API_BASE_URL || `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}`}/api/candidate/update-profile?candidateId=${candidateId}`,
        payload
      );
  ...
  const handleDownloadResume = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL || `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}`}/${profile.resumePath}`,
        { responseType: "blob" }
      );
