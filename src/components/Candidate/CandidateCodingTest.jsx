    ...
    const fetchData = async () => {
      ...
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL || `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}`}/api/interviews/candidate/coding/${candidateId}`
        );
        const data = res.data;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL || `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}`}/api/interviews/coding/submit/${sessionId}`,
        {
          code,
